	...
	for _, sv := range *response {
		settings, diag := datasource_servers.NewSettingsValueMust(
			...
			map[string]attr.Value{
				...
				"is_sentinel_enabled":                   optionalBool(sv.Settings.IsSentinelEnabled),
				...
				"sentinel_metrics_history_days":         optionalInt64(sv.Settings.SentinelMetricsHistoryDays),
				"sentinel_metrics_refresh_rate_seconds": optionalInt64(sv.Settings.SentinelMetricsRefreshRateSeconds),
				"sentinel_token":                        optionalString(sv.Settings.SentinelToken),
