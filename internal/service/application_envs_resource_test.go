		...
		Steps: []resource.TestStep{
			...
			{ // ImportState testing
				...
				ImportStateCheck: func(is []*terraform.InstanceState) error {
					...
					if is[0].Attributes["uuid"] != acctest.ApplicationUUID {
						return fmt.Errorf("expected uuid to be %s, got %s", acctest.ApplicationUUID, is[0].Attributes["uuid"])
					}
					return nil
				},
			},
...
func TestApplicationEnvsResourceSchema(t *testing.T) {
	...
	rs := service.NewApplicationEnvsResource()
